...
docker run --name=php-fpm  --volumes-from approot --volumes-from php_config_root -d --add-host localhost:172.17.42.1 --add-host amazontest:54.223.196.153 dockerhub:5000/php-fpm
docker run --name=nginx --volumes-from approot --volumes-from nginx_config_root  --volumes-from nginx_log_root --link php-fpm:php -p 8081:8081 -p 8484:8484 -p 80:80 -d dockerhub:5000/nginx
